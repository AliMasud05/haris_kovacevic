...
import { useGetMeWithProgressQuery } from "@/redux/api/authApi";
...
export default function UserDetailsPage() {
  ...
  const { data: userDatawqithprogress,isLoading:userDataLoading } = useGetMeWithProgressQuery({});
  const userData = userDatawqithprogress?.data;
  console.log(userDatawqithprogress,"user data with progress")
  const enrollments = userDatawqithprogress?.data?.enrollments;
  console.log(enrollments,"enrollments")
    ...
    const totalPaymentAmount = userData?.payments?.reduce((total: number, payment: Payment) => {
        return total + (payment.paymentAmount || 0);
    }, 0) || 0;

    //total discount
    interface EnrollmentWithDiscount extends Enrollment {
      discount?: number;
    }

    const totalDiscount: number =
      (userData?.enrollments as EnrollmentWithDiscount[] | undefined)?.reduce(
      (total: number, enrollment: EnrollmentWithDiscount) => {
        return total + (enrollment.discount || 0);
      },
      0
      ) || 0;
                    ...
                    >
                      ...
                      <div>
                        <div className=" p-4  ">
                          <CircularProgress
                            progress={
                              enrollment?.progressSummary.overallProgress
                            }
                          />
