...
const AllCoursesHome = () => {
  const { data: courses = [], isLoading, isError } = useGetCoursesQuery({});
  console.log("AllCoursesHome courses", courses);

  const ongoingCourses = courses?.data?.filter(
    (course: TCourse) => course.status === "ONGOING"
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8 mb-12">
      {ongoingCourses?.slice(0, 3).map((course: TCourse) => (
