...
import PaypalPaymentForm from "./paypal/PaypalPaymentForm";
...
export default function CoursePaymentInfo() {
  ...
  return (
    <div className="container  lg:w-2xl mx-auto p-6">
        ...
        <div
          className={`flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${
            selectedMethod === "paypal"
              ? "border-text-secondary text-secondary"
              : "border-gray-200"
          }`}
      ...
      {selectedMethod === "card" && <StripePaymentForm />}
      {selectedMethod === "paypal" && <PaypalPaymentForm /> }
    </div>
  );
}
